...
int main(void)
{
    ...
    for (int n = 0; n < N; n++)
    {
        ...
        az[n] = 0.05f + 0.30f * sinf(2.0f * (float)M_PI * 0.5f * t) + 0.10f * sinf(2.0f * (float)M_PI * 7.0f * t); // mix
    }
    ...
    if (mcount_x > 1)
        printf("      mode=% .5f (count=%d)\n", modex, mcount_x);
    else
        printf("      mode: none (no repeated values)\n");
